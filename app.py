...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v11.1", layout="wide")
...
PARAM_GROUPS = {
    ...
    "Complejidad": ["DFA_alpha1", "DFA_alpha2", "D2", "ApEn", "SampEn"],
...
def correct_artifacts_kubios_like(rr, level="none", window=5):
    """
    Corrección aproximada tipo Kubios mediante mediana local + interpolación.

    v10.0 corrige un error lógico:
    En versiones previas, si 'very strong' detectaba >30% de artefactos,
    se anulaba toda la corrección:
        if np.mean(artifacts) > 0.30: artifacts[:] = False

    Eso hacía que 'very strong' pareciera no corregir, mientras 'strong' sí.
    Ahora:
    - no se anula la corrección por superar 30%,
    - se usa interpolación cuando quedan suficientes puntos buenos,
    - si la detección es masiva, se protege conservando puntos buenos mediante
      un criterio robusto adicional de rango fisiológico y mediana.
    """
    ...
    # Mediana local robusta.
    local = pd.Series(rr).rolling(window=window, center=True, min_periods=1).median().to_numpy()

    # Criterio 1: diferencia absoluta respecto a mediana local.
    artifacts_local = np.abs(rr - local) > th

    # Criterio 2: saltos bruscos latido a latido.
    diff = np.abs(np.diff(rr, prepend=rr[0]))
    artifacts_jump = diff > th

    # Criterio 3: límites fisiológicos generales ya en segundos.
    artifacts_phys = (rr < 0.30) | (rr > 2.00)

    artifacts = artifacts_local | artifacts_jump | artifacts_phys

    # Si very strong marca demasiados puntos, NO se apaga la corrección.
    # En su lugar, se relaja mínimamente para no destruir una señal muy irregular.
    if level == "very strong" and np.mean(artifacts) > 0.70:
        th_relaxed = 0.08
        artifacts = (np.abs(rr - local) > th_relaxed) | (diff > th_relaxed) | artifacts_phys

    if level == "strong" and np.mean(artifacts) > 0.60:
        th_relaxed = 0.18
        artifacts = (np.abs(rr - local) > th_relaxed) | (diff > th_relaxed) | artifacts_phys
    ...
    if np.sum(good) >= 2 and np.sum(artifacts) > 0:
        ...
        # Seguridad: si todo se marca como artefacto, no inventar toda la señal.
        # Se deja sin modificar y se avisa en artifact_info.
        artifacts[:] = False
        rr_corr = rr.copy()
    ...
    return rr_corr, artifacts, {
        ...
        "threshold_seconds": float(th),
        "note": "corrección activa; no se desactiva por porcentaje alto",
    }
...
def _sample_entropy_core(x, m=2, r=None, zero_policy="nan", theiler_window=0):
    ...
    Ajuste v11.0:
    - permite ventana de Theiler para excluir comparaciones próximas.
    """
    b, a = _sample_entropy_counts(x, m=m, r=_resolve_entropy_radius(cg, ref, r_factor=r_factor, radius_mode=radius_mode), theiler_window=theiler_window)
...
st.title("VRC / HRV RRi Analyzer Pro v11.1")
